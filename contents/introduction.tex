...
本文档使用\PRCthesis{}本身作为模板，即\pkg{PRCthesis.cls}, \pkg{PRCthesis.sty}和\pkg{PRCthesis.cfg}，旨在展现\PRCthesis{}的使用方法。请结合\PRCthesis{}用户手册和本文档源代码进行学习和操作。
...
出于性能和管理方面的考虑，\PRCthesis{}使用分布式的源文件方案，将论文的各个部分（通常以章为单位）分散到tex文件中，然后在主文档\pkg{main.tex}中统一处理。\figref{fig:filetree}展示了一个可能的文件目录情况。
\begin{figure}[htb]
	\dirtree{%
		.1 \myfolder{pink}{工作文件夹}.
		.2 \myfolder{cyan}{\pkg{PRCthesis.cls}}.
		.2 \myfolder{cyan}{\pkg{PRCthesis.cfg}}.
		.2 \myfolder{cyan}{\pkg{PRCthesis.sty}}.
